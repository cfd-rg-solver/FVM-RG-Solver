...
double OneTempApprox::getGamma(macroParam& point)
{
    ...
    size_t component = 0; // need to be fixed if we're considering multicomponent gas
    double Cv_tr = 3.0 / 2 * kB / point.mixture.mass(component);
    double Cv_rot = 0;
    double Cv_vibr = 0;
    if (point.mixture.components[component].numberAtoms != 1) {
        double Cv_rot = kB / point.mixture.mass(component);
        double Cv_vibr = avgVibrEnergyDiff(point, component) / point.mixture.mass(component);
    }
