...
//    return;
//}
void HLLESolver::computeFlux(SystemOfEquation *system, double dt, double dh)
{
    toMaxVelocity(-1); // для обнуления максимальной сигнальной скорости
    #pragma omp parallel for schedule(static)
    for(int i = 0 ; i < system->numberOfCells-1; i++)
...
void HLLSimple::computeFlux(SystemOfEquation *system, double dt, double dh)
{
    ...
    for(int i = 0 ; i < system->numberOfCells - 1; i++)
    {
        ...
        double H0, H1, c0, c1, u0, u1, v0,v1,V0,V1, rho0, rho1, u_avg,v_avg, H_avg, c_avg, SL, SR;
        ...
        SL = (std::min)({u_avg - c_avg, u0 - c0});
        SR = (std::max)({u_avg + c_avg, u1 + c1});

        toMaxVelocity(max(fabs(SL),fabs(SR)));
        if(SL >= 0)
        {
            for(size_t j = 0; j < system->systemOrder; j++)
            {
        ...
        else{ std::cout << "omg again "<< std::endl; }
    }
}

void ExacRiemanSolver::computeFlux(SystemOfEquation *system, double dt, double dh)
{
    #pragma omp parallel for schedule(static)
    for(int i = 0 ; i < system->numberOfCells - 1; i++)
    {
